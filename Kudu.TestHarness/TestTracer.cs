...
namespace Kudu.TestHarness
{
    public static class TestTracer
    {
        ...
        public static void Trace(DateTime messageDateTime, string messageFormat, params object[] args)
        {
            ...
            message = XmlUtility.Sanitize(String.Format(CultureInfo.CurrentCulture, "{0}Z {1}", messageDateTime.ToUniversalTime().ToString("s"), message.Replace("\n", "\n\t")));
        ...
        public static string GetTraceString(string typeName, string testName)
        {
            var strb = (StringBuilder)CallContext.LogicalGetData(TestTracer.DataSlotName);
            lock (strb)
            {
